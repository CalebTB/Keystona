...
- **`FractionallySizedBox` for proportional skeleton bars**: Use `FractionallySizedBox(widthFactor: f, alignment: Alignment.centerLeft)` for percentage-width shimmer bars — `LayoutBuilder` crashes inside `SliverFillRemaining(hasScrollBody: false)` due to intrinsic dimension limitations; `FractionallySizedBox` achieves the same result through the constraint system → `lib/features/home_profile/widgets/home_profile_skeleton.dart`
- **Extension methods on `String` for enum display labels**: `extension UtilityTypeLabel on String` / `ContactCategoryLabel` / `PolicyTypeLabel` — extend `String` directly so label lookup works on any raw DB value without defining a Dart enum or a widget-local switch → `lib/features/emergency/models/utility_shutoff.dart`
- **Plain class for composite overview models**: Non-persisted multi-source models (e.g. `EmergencyHubOverview`) use a plain Dart `class const` constructor — never Freezed since no `fromJson` is needed; assembled in-memory from parallel queries; same pattern as `TaskDetail` → `lib/features/emergency/models/emergency_hub_overview.dart`
- **Quick-action button for non-tab pushed screens**: Non-tab screens (Emergency Hub) need an explicit entry point on an existing tab screen — add a styled `GestureDetector` container at the bottom of the relevant tab's content list; always verify a new pushed-route screen has a reachable entry point before closing the issue → `lib/features/home_profile/screens/home_profile_screen.dart`
- **Local `picked` variable for `CupertinoDatePicker`**: `CupertinoDatePicker` fires `onDateTimeChanged` on every drum scroll — capture into a local `DateTime picked = initial` and only commit to state in the "Done" button handler; prevents intermediate partial-scroll values from dirtying form state → `lib/features/emergency/screens/insurance_form_screen.dart`
- **`showDocumentLinkPicker()` returning typed record**: Cross-feature document picker exposed as a top-level function returning `({String id, String name})?` — caller gets both ID and display name in one call without a secondary lookup; same `PickerOption` record pattern but for cross-feature use → `lib/features/emergency/widgets/document_link_picker.dart`
- **`StatefulWidget` (not `ConsumerStatefulWidget`) for pickers that use `SupabaseService` directly**: Document picker sheet queries Supabase via the static `SupabaseService.client` — no Riverpod `ref` required; lighter widget with `StatefulWidget` + direct client calls → `lib/features/emergency/widgets/document_link_picker.dart`
- **`Flexible(child: ListView.separated(shrinkWrap: true))` inside modal Column**: Allows a list inside a `Column` in a constrained modal to take remaining space and scroll — `Flexible` lets the list shrink if few items, `shrinkWrap: true` prevents unconstrained height error → `lib/features/emergency/widgets/document_link_picker.dart`
- **`_parseCurrency()` helper for monetary text fields**: Strip `$` and `,` before `double.tryParse` — enables free-form currency input that round-trips cleanly to a Supabase NUMERIC column without rejecting comma-formatted values → `lib/features/emergency/screens/insurance_form_screen.dart`
- **Computed display getters on plain RPC model**: RPC result models (not persisted) carry UI logic directly as instance getters (`healthColor`, `healthLabel`, `barFraction`, `isEndOfLife`) — the model is the single source of truth for both data parsing and display; no extension methods or widget-local switches needed → `lib/features/home_profile/models/system_lifespan_entry.dart`
- **Contextual cost banner via `if` guard in sliver list**: Urgency banner (`_ReplacementCostBanner`) computed before the sliver tree by folding over the filtered urgent list — only inserted as a `SliverToBoxAdapter` when `totalReplacementCost > 0`; zero conditional widget tree overhead when no urgent systems → `lib/features/home_profile/screens/lifespan_screen.dart`
- **Fixed-width `Container`s for right-side skeleton bars in `Row`**: Skeleton bars that appear at right-side positions inside a `Row` (where parent is unconstrained) use `Container(width: N)` — `FractionallySizedBox` requires a finite parent width; explicit px widths (80, 64) are fine for shimmer placeholders since precision doesn't matter → `lib/features/home_profile/widgets/lifespan_skeleton.dart`
- **Pattern B Showcase empty state with ghosted example cards**: Two `_ExampleProjectCard` widgets at `Opacity(opacity: 0.5)` inside a `Column` — non-interactive previews of real content; CTA button below the ghosted cards; exact §2.4 copy; use when the feature is visual enough that seeing examples is more compelling than a plain illustration → `lib/features/projects/widgets/project_empty_state.dart`
- **`String? _activeFilter` with toggle-to-clear filter chips**: `null` = show all; chips compare `activeFilter == s.value` — tapping an active chip calls `onFilterChanged(null)`, tapping inactive calls `onFilterChanged(s.value)`; driven from `ConsumerStatefulWidget` state so filter survives provider rebuilds; `_NoResultsState` shown in list area when filter yields zero results → `lib/features/projects/screens/projects_screen.dart`
- **Budget progress bar with overflow color on `LinearProgressIndicator`**: `value: (spent / estimated).clamp(0.0, 1.0)` for bar fill; `color: pct >= 1.0 ? AppColors.error : AppColors.deepNavy` on both the indicator and the percentage label — visually alerts user when over budget without a separate state → `lib/features/projects/widgets/project_card.dart`
...
- **`FractionallySizedBox` over `LayoutBuilder` for skeleton width fractions**: Chose `FractionallySizedBox` because `LayoutBuilder` cannot respond to intrinsic dimension queries from `SliverFillRemaining(hasScrollBody: false)` — same proportional result, no crash
- **`Future.wait([q1, q2, q3, q4])` for hub overview fetch**: Emergency Hub needs 4 independent data sets (shutoffs, fav contacts, all contacts, insurance) — parallel `Future.wait` reduces total latency to the slowest single query instead of sum of all four
- **`typedef NoEmergencyPropertyException = _NoPropertyException`**: Private exception exported via `typedef` — sentinel type; downstream screens catch the public alias without coupling to the private symbol
- **Top-level `show*Picker()` function over `static show()` for cross-feature pickers**: When a picker widget needs platform-branching (`showCupertinoModalPopup` vs `showModalBottomSheet`) AND returns data from a cross-feature domain, a top-level function is cleaner than a static method — call site reads `showDocumentLinkPicker(context)` without importing the widget class → `lib/features/emergency/widgets/document_link_picker.dart`
- **`CupertinoDatePicker` wrapped in `Material(type: MaterialType.transparency)` + height `Container`**: Inside `showCupertinoModalPopup`, a fixed-height `Container` (300px) with a header Row (Cancel/Done) + `Expanded(CupertinoDatePicker)` gives a proper bottom-sheet date picker on iOS — no `SizedBox.expand` or `Flexible` needed, `Material` ancestor prevents downstream Material widget assertion errors → `lib/features/emergency/screens/insurance_form_screen.dart`
- **`@JsonKey(name:)` to avoid Freezed mixin naming conflict**: When a DB column name conflicts with a Freezed-generated mixin property (e.g. `model`), give the Dart field a different name and annotate with `@JsonKey(name: 'db_column_name')` — `@JsonKey(name: 'model') String? modelNumber` keeps JSON mapping correct without touching the generated mixin → `lib/features/home_profile/models/system.dart`
- **`@JsonEnum(valueField:)` with `get label` on Dart enums**: Map Postgres enum strings using `@JsonEnum(valueField: 'value')` + explicit `final String value` field; add `String get label` for UI display — cleaner than String extension methods when you own the enum type → `lib/features/home_profile/models/system.dart`
- **Non-fatal Edge Function call after insert**: Wrap secondary Edge Function calls (task generation, notifications) in `try {} catch (_) {}` with a "Non-fatal" comment — the primary DB write must never be blocked by downstream function failures; user can trigger secondary action manually if needed → `lib/features/home_profile/providers/systems_provider.dart`
- **`BoxConstraints(minHeight:)` for bordered cards**: Cards with `Border.all()` must use `constraints: const BoxConstraints(minHeight: AppSizes.cardMinHeight)` instead of `height: 72` — `Border.all(width: 1)` draws inward consuming 2px from content area; fixed height + padding + border yields less space than expected → `lib/features/home_profile/widgets/system_card.dart`
- **`abstract final class` picker catalog with `(value:, label:)` records**: For string-based DB enum options, define `static const all = [(value: 'plumber', label: 'Plumber'), ...]` and `static String labelFor(String value)` in an `abstract final class` — drives pickers with a single list, handles display via `labelFor()`, and documents the exact DB enum values in one place; preferred over extension methods on String when a catalog of all options is also needed → `lib/features/emergency/models/emergency_contact.dart`
...
- **`LayoutBuilder` inside `SliverFillRemaining(hasScrollBody: false)` crashes**: `SliverFillRemaining` calls `computeMaxIntrinsicHeight` on its child — `LayoutBuilder` cannot respond to intrinsic dimension queries, causing a cascade of render tree errors (`LayoutBuilder does not support returning intrinsic dimensions` → `!semantics.parentDataDirty` → `debugFrameWasSentToEngine`) → replace any `LayoutBuilder` usage in skeleton bar widgets with `FractionallySizedBox`
- **`addPostFrameCallback` is required in ALL skeleton `initState` methods, not just pushed routes**: Tab-root skeletons also crash on some startup paths with `debugFrameWasSentToEngine` — apply `WidgetsBinding.instance.addPostFrameCallback((_) { if (mounted) _ctrl.repeat(reverse: true); })` universally in every skeleton; never call `.repeat()` directly in `initState`
- **Non-tab pushed-route screens must have a reachable entry point**: Emergency Hub was routed and building correctly but completely unreachable — no button existed anywhere in the app; always verify new pushed-route screens have a visible entry point on an existing tab screen before closing the issue
- **`@JsonKey` model fix may not propagate to downstream worktrees despite merging main**: After the `is_24x7` `@JsonKey` fix was merged into main from #46, merging main into keystona-47 didn't surface the fix in the running app — either build_runner ran against a stale source or the source file wasn't overwritten. Prevention: after merging main into any worktree that uses a recently-fixed model, run `grep -r 'is24x7\|is_24x7' lib/` to verify the source is correct BEFORE running build_runner; do not trust that auto-merge applied the fix
- **Controllers in data model for inline editable rows**: For dynamic lists where each row has inline text inputs (e.g. circuit directory), own `TextEditingController`s in the row's data container class with a `dispose()` method — parent state disposes on removal, controllers survive parent rebuilds; `_IOSField`/`_AndroidField` receive the controller directly instead of `initialValue` → `lib/features/emergency/widgets/circuit_directory_editor.dart`
- **`Border.all()` draws inward, stealing 2px of content height**: `Border.all(width: 1.0)` in `BoxDecoration` renders 1px inside the box bounds on each side — `Container(height: 72, padding: EdgeInsets.all(16), decoration: BoxDecoration(border: Border.all()))` gives only 38px for content (72 - 32 - 2), not 40px; a two-line text column + 2px spacer = 39px → 1px overflow → use `constraints: BoxConstraints(minHeight:)` so the card expands instead of clipping
- **`ListView` inside `SliverFillRemaining(hasScrollBody: false)` crashes**: `SliverFillRemaining` asks its child for intrinsic height — `ListView` is a `RenderViewport` which throws "RenderViewport does not support returning intrinsic dimensions", cascading into `!semantics.parentDataDirty` → replace the skeleton's `ListView` with a `Column` (skeleton items are fixed and don't need to scroll); same root cause as the LayoutBuilder lesson but different widget
- **Feature worktrees leave code uncommitted**: Agents build code in worktrees but don't always `git add` / commit — `git status --short` shows `??` untracked files for the entire feature; `git merge main` aborts with "untracked files would be overwritten by merge" → always run `git status --short` and commit all untracked feature files before merging main into a feature branch during merge train
- **DB enum values don't match model enum values**: Agent generated `ApplianceCategory` with appliance-type values (`refrigerator`, `washer_dryer`) but DB `appliance_category` enum uses room-based values (`kitchen`, `laundry`, `climate`) — `fromJson` throws on every row; always verify actual DB enum values with `SELECT unnest(enum_range(NULL::your_enum))` before writing model enum code → `lib/features/home_profile/models/appliance.dart`
- **Same wrong column name in multiple providers**: `expected_lifespan_years` appeared in both `appliances_provider.dart` (`_kColumns`) and `appliance_detail_provider.dart` (`_kApplianceColumns`) — fixing one didn't fix the other; always search all providers for the old column name when a column is renamed → use `grep -r "old_column_name" lib/`
- **NOT NULL column missing from insert payload**: `appliance_type` is NOT NULL in the DB but was never included in the form's `_buildFields()` — inserts silently fail at the DB constraint; always check `SELECT column_name FROM information_schema.columns WHERE table_name = 'X' AND is_nullable = 'NO'` before writing insert code
- **`TextEditingController` created in `build()` resets cursor to 0 on every rebuild**: `StatelessWidget` that calls `TextEditingController(text: value)` inside `build()` creates a fresh controller each time the parent rebuilds — cursor jumps to position 0, so typing appends before existing text (appears backwards), and deleting requires re-focus each time; fix: own controllers in a `StatefulWidget` state or in the data model class → `lib/features/emergency/widgets/circuit_directory_editor.dart`
- **`field_rename: snake` does not convert fields with no uppercase letters**: `json_serializable`'s snake_case conversion only inserts underscores before uppercase letter boundaries — a field named `is24x7` has no uppercase letters so stays `is24x7` in JSON, but the DB column is `is_24x7`; this causes `required bool` to receive `null` and throws at runtime; fix with explicit `@JsonKey(name: 'is_24x7')` on the Freezed factory parameter → `lib/features/emergency/models/emergency_contact.dart`
- **`@JsonKey` on a Freezed factory constructor parameter triggers a false-positive `invalid_annotation_target` lint**: The analyzer doesn't know that Freezed uses factory constructor parameters as field declarations — it warns even though the annotation IS applied correctly by the generator; suppress with `// ignore_for_file: invalid_annotation_target` at the top of the model file → `lib/features/emergency/models/emergency_contact.dart`
- **`ref.invalidateSelf(); await future` propagates refetch failures as write failures**: After a successful DB insert, `await future` re-runs the provider's full `build()` — if that refetch queries multiple tables (e.g. `_fetchOverview()` hits utility_shutoffs + contacts + insurance_info in parallel) and any query fails, the exception propagates all the way back to the caller; the form shows "couldn't save" even though the row was inserted; always verify the hub overview fetch succeeds independently before debugging the insert itself
- **`FractionallySizedBox` bare in a `Row` crashes with infinite width**: `Row` gives unconstrained width to direct children — `FractionallySizedBox(widthFactor: 0.35)` requires a finite parent width to compute its fraction, so it crashes with "BoxConstraints forces an infinite width"; fix: wrap in `Expanded` (for proportional sizing) or replace with `Container(width: N)` (for fixed skeleton bars); the first child in the name row works because it IS wrapped in `Expanded`
- **Merging a branch with new `@riverpod` providers into another worktree requires build_runner**: After merging `main` (which landed #48's `lifespan_provider.dart`) into the #49 worktree, `lifespanProvider` was undefined — the `.g.dart` file didn't exist in that worktree; always run `dart run build_runner build --delete-conflicting-outputs` after any merge that brings in new `@riverpod`-annotated providers or `@freezed` models
- **iOS `Stack + Positioned` FAB must be gated when the empty state has its own CTA**: `CupertinoPageScaffold` has no `floatingActionButton` slot so a FAB requires `Stack + Positioned` — but when the empty state renders its own CTA button, the floating FAB overlays it, giving two competing tap targets; gate with `if (projectCount > 0)` on the `Positioned` child so FAB only appears when real content exists → `lib/features/projects/screens/projects_screen.dart`
