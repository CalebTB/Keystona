---
name: compound
description: Extract and document patterns, decisions, and failures from recently completed work to make future development easier. Use this for comprehensive learning extraction after completing features, fixing complex bugs, or at the end of a sprint. Triggers on 'compound', 'compound learnings', 'extract learnings', 'document patterns', or when you need a thorough analysis of what was learned from recent work. This is the full version - use 'quick-compound' for a faster extraction.
---
...
### Step 5: Update CLAUDE.md Automatically

Read CLAUDE.md and append learnings to the appropriate sections:

1. Read the current CLAUDE.md file
2. For each pattern found, append to the `## Patterns` section with full details:
   ```markdown
   - **[Pattern Name]**: [When to use] → Implementation in `[file path]` → [Related patterns]
   ```
3. For each decision made, append to the `## Decisions` section:
   ```markdown
   - **[Decision]**: Chose [X] over [Y] because [reason]. Context: [context]. Trade-offs: [consequences]
   ```
4. For each lesson learned, append to the `## Lessons` section:
   ```markdown
   - **[Issue]**: [Symptom] → Root cause: [cause] → Prevention: [how to prevent]
   ```
...
- [ ] Update feature-specific docs for [pattern]
